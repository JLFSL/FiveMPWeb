<?php 
	...
	while ($row = $articles->fetch()) {
		...
		<div class="row">
			...
			<div class="col-sm-8">
				<h1><a class="deco-none" href="/article/' . $row["id"] . '/' . $row["title"] . '/0"><b>' . $row["title"] . '</b></a></h1>
				...
				<br>
				<a href="/article/' . $row["id"] . '/' . $row["title"] . '/0"><button type="button" class="btn btn-danger btn-sm pull-xs-right red-div">Read More</button></a>
			</div>
		</div>';
	}
?>
